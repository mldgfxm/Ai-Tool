...
App routing means letting AIsnake route a specific application's API requests through the local routing service.
...
When routing is enabled, AIsnake modifies the app's configuration file to point the API endpoint to the local routing service.
...
When disabling routing, AIsnake will:
...
Before enabling routing, AIsnake backs up the original configuration and restores it when disabled.

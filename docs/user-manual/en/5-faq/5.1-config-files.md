...
## AIsnake Data Storage

### Storage Directory

Default location: `~/.aisnake/`
...
```
~/.aisnake/
├── aisnake.db      # SQLite database (SSOT)
...
`aisnake.db` is a SQLite database that stores:
...
| Field | Description |
|-------|-------------|
| `models.providers` | Provider configuration (mapped to AIsnake's "providers") |
...
AIsnake's priority when modifying configurations:

1. **AIsnake Database** - Single source of truth (SSOT)
...
- CLI tool configuration files (will be backfilled by AIsnake)
- AIsnake's `settings.json`

### Not Recommended to Edit Manually

- `aisnake.db` database file
- Backup files
...
AIsnake v3.7.0 migrated from JSON files to SQLite:

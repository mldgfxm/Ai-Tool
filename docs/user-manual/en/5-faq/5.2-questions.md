...
AIsnake for macOS is code-signed and notarized by Apple. You can download and install it directly without any additional steps. If you encounter issues, try downloading the latest version from the [Releases page](https://github.com/farion1231/aisnake/releases).
...
2. Add AIsnake to your antivirus software's whitelist
...
**Solutions**:
1. Check if the `~/.aisnake/` directory exists
2. Restore from backup: `~/.aisnake/backups/`
...
**Solutions**:
1. Confirm the file was exported by AIsnake
...
**Windows**:
- Check taskbar settings to ensure the AIsnake icon is not hidden
...
2. Restart the app
3. Delete `~/.aisnake/settings.json` to reset settings
...
3. If using Homebrew: `brew upgrade --cask aisnake`

## Lightweight Mode

### How to Enter Lightweight Mode?

Toggle "Lightweight Mode" from the system tray menu. The main window closes, and AIsnake runs as a tray-only app. Toggle again or click "Open main window" to exit.
...
Yes. Starting from v3.13.0, AIsnake covers all window re-show paths (normal launch, deep links, singleton activation, tray `show_main`, and Lightweight Mode return). Clicking a `aisnake://` link **rebuilds the main window on demand** and displays the import confirmation dialog. The first open is slightly slower than normal state (window rebuild required), but subsequent switches return to normal speed.
...
1. Visit [GitHub Issues](https://github.com/farion1231/aisnake/issues)
...
- macOS/Linux: `~/.aisnake/logs/`
- Windows: `%APPDATA%\aisnake\logs\`

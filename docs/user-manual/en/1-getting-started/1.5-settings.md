...
This section describes how to configure AIsnake according to your preferences.
...
AIsnake supports three languages:
...
When enabled, AIsnake automatically runs when the system starts.
...
Starting from v3.13.0, AIsnake adds **Lightweight Mode** — a **tray-only** running state that minimizes desktop footprint when idle.
...
**How to exit**: Click **Open Main Window** from the tray menu, or trigger AIsnake via deep link / relaunch. The window is **rebuilt on demand**, with state preserved.
...
> **Use case**: If AIsnake runs in the background for long periods and you mainly switch providers via the tray menu, enabling Lightweight Mode significantly reduces memory usage.
...
When enabled, AIsnake automatically syncs the configuration to the VS Code Claude Code extension (writes `primaryApiKey` to `~/.claude/config.json`) when switching providers.
...
Choose the terminal application that AIsnake uses when opening a terminal.
...
The storage location for AIsnake's own data, defaulting to `~/.aisnake/`.
...
Configure AIsnake's outbound HTTP/HTTPS proxy, applicable for scenarios where external API access requires a proxy.
...
When an interval is set, AIsnake automatically backs up the database on schedule. Older backups beyond the retention count are automatically removed.
...
| Remote Directory | Remote storage path (default: `aisnake-sync`) |
...
- AIsnake automatically syncs the database to WebDAV at the configured interval
- Sync status is displayed in the panel

#### Remote Snapshot Info

Before downloading, AIsnake displays details about the remote snapshot:
- Protocol version (v2)
...
Displays the current AIsnake version number, with support for:

...
Only download AIsnake from **[aisnake.io](https://aisnake.io)**, **[GitHub Releases](https://github.com/farion1231/aisnake/releases)**, or the project source repository. Any "AIsnake" site or client that asks for payment, top-ups, or login credentials is not official.
...
The CLI tools managed by AIsnake (Claude Code, Codex, Gemini CLI) require a Node.js environment.
...
1. Visit the [Releases page](https://github.com/farion1231/aisnake/releases)
...
2. Open the DMG, or extract the zip to get `AIsnake.app`
3. Drag it to the Applications folder

### Signed and Notarized

AIsnake for macOS is signed and notarized by Apple. You can install and open it directly — no extra steps needed.
...
After installation, launch AIsnake:

1. The app window displays correctly
2. A AIsnake icon appears in the system tray
3. The app switcher shows enabled managed apps, and you can switch to the target app panel

## Auto Update

AIsnake includes built-in auto-update functionality:
...
- Move `AIsnake.app` to Trash
- Optional: Delete the configuration directory `~/.aisnake/`

### Linux

```bash
# Debian/Ubuntu
sudo apt remove aisnake

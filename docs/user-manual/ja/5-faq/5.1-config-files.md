...
`aisnake.db` は SQLite データベースで、以下を保存しています：
...
| フィールド | 説明 |
|------|------|
| `models.providers` | プロバイダー設定（AIsnake の「プロバイダー」にマッピング） |
| `env` | 環境変数設定 |
...
AIsnake が設定を変更する際の優先順位：

1. **AIsnake データベース** - 単一事実源 (SSOT)
...
- CLI ツールの設定ファイル（AIsnake がバックフィルする）
- AIsnake の `settings.json`

### 手動編集を推奨しないもの

- `aisnake.db` データベースファイル
- バックアップファイル
...
AIsnake v3.7.0 で JSON ファイルから SQLite に移行しました：

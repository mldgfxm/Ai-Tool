...
AIsnake は **[aisnake.io](https://aisnake.io)**、**[GitHub Releases](https://github.com/farion1231/aisnake/releases)**、またはプロジェクトのソースリポジトリからのみ入手してください。支払い、チャージ、ログイン情報の入力を求める「AIsnake」サイトやクライアントは公式ではありません。
...
AIsnake が管理する CLI ツール（Claude Code、Codex、Gemini CLI）には Node.js 環境が必要です。
...
1. [Releases ページ](https://github.com/farion1231/aisnake/releases) にアクセス
...
2. DMG を開く、または zip を展開して `AIsnake.app` を取得
3. 「アプリケーション」フォルダにドラッグ

### 署名・公証済み

AIsnake の macOS 版は Apple のコード署名と公証を受けています。追加の操作なしで直接インストールして開くことができます。
...
インストール完了後、AIsnake を起動します：

1. アプリウィンドウが正常に表示される
2. システムトレイに AIsnake のアイコンが表示される
3. アプリ切り替えに有効化された管理対象アプリが表示され、目的のアプリパネルへ切り替えられる

## 自動更新

AIsnake には自動更新機能が内蔵されています：

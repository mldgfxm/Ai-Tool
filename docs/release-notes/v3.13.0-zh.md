...
- **Skills 发现与批量更新**：基于 SHA-256 内容哈希的更新检测、单项和"全部更新"批量操作、`skills.sh` 表搜索集成，以及 AIsnake 与 `~/.agents/skills` 的存储位置切换
...
新增仅托盘运行模式，显著降低 AIsnake 空闲时的桌面占用。
...
- **存储位置切换**：在 AIsnake 存储和 `~/.agents/skills` 之间切换而不丢失 skill 状态
...
新安装用户首次打开时显示一次性欢迎对话框，引导了解 AIsnake 工作流程。

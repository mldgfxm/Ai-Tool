...
"#
    ...
    let auth_value: serde_json::Value =
        read_json_file(&aisnake_lib::get_codex_auth_path()).expect("read auth.json");
    assert_eq!(
    ...
    let config_text =
        std::fs::read_to_string(aisnake_lib::get_codex_config_path()).expect("read config.toml");
    assert!(
...
"#
    ...
    assert_eq!(
        ...
        Some("custom"),
        "live Codex third-party model_provider should use the AIsnake history bucket"
    );
...
fn provider_service_switch_codex_official_accounts_write_auth_json() {
    ...
    let auth_b: serde_json::Value =
        read_json_file(&aisnake_lib::get_codex_auth_path()).expect("read auth B");
    assert_eq!(
        auth_b
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
    ...
    let auth_a: serde_json::Value =
        read_json_file(&aisnake_lib::get_codex_auth_path()).expect("read auth A");
    assert_eq!(
        auth_a
            .pointer("/tokens/access_token")
            .and_then(|v| v.as_str()),
...
fn switch_packycode_gemini_updates_security_selected_type() {
    ...
    // Gemini security settings are written to ~/.gemini/settings.json, not ~/.aisnake/settings.json

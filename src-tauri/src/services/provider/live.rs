...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        AppType::Codex => {
            ...
            // `modelCatalog` is a aisnake private field that lives only in
            // the DB SSOT plus the `aisnake-model-catalog.json` projection
...
/// into the AIsnake database. Each provider found will be added to the

...
export interface Provider {
  ...
  // 新增：是否为商业合作伙伴
  isPartner?: boolean;
  // 可选：供应商元数据（仅存于 ~/.aisnake/config.json，不写入 live 配置）
  meta?: ProviderMeta;
  // 图标配置
...
// Skill 存储位置
export type SkillStorageLocation = "aisnake" | "unified";
...
// 存储在本地 ~/.aisnake/settings.json，不随数据库同步
export interface Settings {
  ...
  // Skill 存储位置：aisnake（默认）或 unified（~/.agents/skills/）
